...
void dump_fog_param_dbg(fog_parameter_t* fog_inst, alt_u8 ch) {
    ...
    snprintf(buffer + offset, sizeof(buffer) - offset, "}\n"); // Close JSON structure
    DEBUG_PRINT("%s", buffer); // Print the formatted JSON string
	send_json_uart_dbg(buffer); // Send the JSON data via UART
}
...
void dump_SN(fog_parameter_t* fog_inst) {
	...
	uartWrite(&fog_inst->sn[0], 4);
	uartWrite(&fog_inst->sn[4], 4);
	uartWrite(&fog_inst->sn[8], 4);

}

void dump_SN_dbg(fog_parameter_t* fog_inst) {

	uartWrite_dbg(&fog_inst->sn[0], 4);
	uartWrite_dbg(&fog_inst->sn[4], 4);
	uartWrite_dbg(&fog_inst->sn[8], 4);

}

void send_json_uart(const char* buffer) {
    while (*buffer) {
		uartPutByte((alt_u8)*buffer);
        buffer++;
    }
}
