#ifndef __COMMON_V2_H
#define __COMMON_V2_H

#include "alt_types.h"
#include "uart_V2.h"  // 使用 uartWrite/uartPrintf + UART buffer 指標
#include "system.h"
...
#ifdef UART_MAIN
    #ifdef UART_DEBUG
        // 當 UART_MAIN 和 UART_DEBUG 都定義時
        #define UART_PRINT(...) do { \
            uartPrintf(__VA_ARGS__); \
            uartPrintf_dbg(__VA_ARGS__); \
        } while (0)
    #else
        // 只有 UART_MAIN 定義時
        #define UART_PRINT(...) uartPrintf(__VA_ARGS__)
    #endif
#else
    #ifdef UART_DEBUG
        // 只有 UART_DEBUG 定義時
        #define UART_PRINT(...) uartPrintf_dbg(__VA_ARGS__)
    #else
        // 兩個都沒有定義時 (或只有 UART_DEBUG 定義而 UART_MAIN 沒有)
        #define UART_PRINT(...)
    #endif
#endif
...
void dump_fog_param_dbg(fog_parameter_t* fog_inst, alt_u8 ch);
void dump_misalignment_param(fog_parameter_t* fog_inst);
void dump_misalignment_param_dbg(fog_parameter_t* fog_inst);
void dump_SN(fog_parameter_t* fog_inst);
void dump_SN_dbg(fog_parameter_t* fog_inst);
void send_json_uart(const char* buffer);
void send_json_uart_dbg(const char* buffer);
