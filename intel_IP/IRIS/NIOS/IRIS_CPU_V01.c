...
int main(void)
{
	...
	while(1){
		...
		/***---x axis--- */
		sensor_data.fog.fogx.err.int_val = IORD(VARSET_BASE, i_var_err_1);
		sensor_data.fog.fogx.step.float_val = moving_average_update(&mz_x, (float)IORD(VARSET_BASE, i_var_step_1));
		/***---y axis--- */
		sensor_data.fog.fogy.err.int_val = IORD(VARSET_BASE, i_var_err_2);
		sensor_data.fog.fogy.step.float_val = moving_average_update(&mz_y, (float)IORD(VARSET_BASE, i_var_step_2));
		/***---z axis--- */
