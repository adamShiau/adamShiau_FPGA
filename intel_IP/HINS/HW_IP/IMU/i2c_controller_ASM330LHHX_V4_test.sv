/*** reversion log
// V2: 2-4-2026 建立
// 新增 i_latch_trigger 來 latch data，需對應修改後的 VaeSet_60 ip [cite: 1, 59]

// V3: 3-10-2026 建立
// 新增 sm_reset = i_ctrl[8] 來重置 SM 到 IDLE [cite: 1, 30, 42]

// V4: 3-17-2026 建立
// 優化 Bus Recovery 機制：當 sm_reset 觸發時，狀態機將執行修復序列
// 1. 自動產生 9 個 SCL 脈衝以強制從機 (Slave) 釋放 SDA 線
// 2. 脈衝結束後發送一個標準 STOP Condition，確保匯流排回歸 IDLE 狀態
// 3. 解決模式切換 (HW <-> CPU) 時可能導致的匯流排死鎖 (Deadlock) 問題
***/

module i2c_controller_ASM330LHHX_V4
(
	input wire 			i_clk,
	input wire 			i_rst_n,
	...
	localparam STOP 		= 	39;
	localparam STOP2 		= 	40;
	// --- 新增狀態定義 for setter 來終止狀態機---
	localparam REC_INIT   = 41;
	localparam REC_CLK    = 42;
	localparam REC_STOP_A = 43;
	localparam REC_STOP_B = 44;

	reg [3:0] rec_cnt; // 9 脈衝計數器
	...
	always @(negedge i2c_clk or negedge i_rst_n) begin
		...
		else if (sm_reset) begin
			i2c_scl_enable <= 1; // 重置時放開 SCL 
		end
		else begin
			case(state)
				READ_ACK_B, READ_ACK2_B, READ_ACK3_B: i2c_scl_enable <= 0;
				IDLE, START, STOP, STOP2, REC_STOP_A, REC_STOP_B: i2c_scl_enable <= 1;
	...
	always @(posedge i2c_clk or negedge i_rst_n) begin
		if(!i_rst_n) begin
			...
			case(state)
				...
				REC_CLK: begin
					if (rec_cnt >= 4'd8) state <= REC_STOP_A;
					else rec_cnt <= rec_cnt + 1'b1;
				end

				REC_STOP_A: state <= REC_STOP_B;

				REC_STOP_B: state <= IDLE; // 完成修復，回到 IDLE
	...
	always @(negedge i2c_clk or negedge i_rst_n) begin
		if(!i_rst_n) begin
			...
			case(state)
				...
				REC_CLK:    sda_out <= 1; // 脈衝期間放開 SDA，讓 Slave 有機會釋放
				REC_STOP_A: sda_out <= 0; // 為 STOP 做準備 (SCL=1, SDA=0)
				REC_STOP_B: sda_out <= 1; // 產生 STOP (SCL=1, SDA: 0->1)
