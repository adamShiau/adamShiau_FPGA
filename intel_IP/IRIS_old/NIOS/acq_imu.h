...
extern alt_u32 get_timer_int();


#define INT_SYNC_IMU 1
#define EXT_SYNC_IMU 2
#define STOP_RUN_IMU 4

const unsigned char KVH_HEADER_2[4] = {0xFE, 0x81, 0xFF, 0x55};
...
void acq_imu (cmd_ctrl_t* rx, my_sensor_t data, fog_parameter_t fog_parameter)
{
    ...
    if(rx->select_fn == SEL_IMU) {
        ...
        if(rx->value == INT_SYNC_IMU) { //internal sync mode
            // DEBUG_PRINT("acq_fog select internal mode\n");
            DEBUG_PRINT("acq_imu select internal mode\n");
            rx->run = 1;
        }
        else if(rx->value == EXT_SYNC_IMU) { //external sync mode
            DEBUG_PRINT("acq_imu select external mode\n");
            rx->run = 1;
        }
        else if(rx->value == STOP_RUN_IMU) { //stop
    ...
    if(rx->run == 1) {        
        g_time[2] = get_timer_int();    
            ...
            memcpy(imu_data, KVH_HEADER_2, 4);
